...
Mixin.prototype = {
  ...
  generateEdDSASessionKeypair: function generateEdDSASessionKeypair() {
    var keypair = _nodeForge["default"].pki.ed25519.generateKeyPair();
  ...
  signEdDSAAuthenticationToken: function signEdDSAAuthenticationToken(uid, sid, privateKey, method, uri, params, scp) {
    if (_typeof(params) === "object") {
      params = JSON.stringify(params);
    } else if (typeof params !== "string") {
      params = "";
    ...
    var jwtBody = _base64url["default"].encode(JSON.stringify(jwtHeader)) + '.' + _base64url["default"].encode(JSON.stringify(jwtPayload));

    var jwtSignature = _nodeForge["default"].pki.ed25519.sign({
      message: jwtBody,
      encoding: 'utf8',
      privateKey: privateKey
    });

    var signedJwt = jwtBody + '.' + _base64url["default"].encode(jwtSignature);
