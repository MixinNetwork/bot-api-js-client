var Mixin = (function (forge, LittleEndian, curve25519Js, uuid, axios, jsSHA) {
  'use strict';
  ...
  var Client = /*#__PURE__*/function () {
    ...
    _createClass(Client, [{
      ...
      value: function signAuthenticationToken(uid, sid, privateKey, method, uri, params, scp) {
        privateKey = Buffer$1.from(privateKey, 'base64');
        ...
        var header = this.util.base64RawURLEncode(Buffer$1.from(JSON.stringify({
          alg: "EdDSA",
          typ: "JWT"
        }), 'utf8'));
        payload = this.util.base64RawURLEncode(Buffer$1.from(JSON.stringify(payload), 'utf8'));
  ...
  var User = /*#__PURE__*/function () {
    ...
    _createClass(User, [{
      ...
      value: function generateSessionKeypair() {
        var keypair = forge__default["default"].pki.ed25519.generateKeyPair();
        var public_key = this.util.base64RawURLEncode(keypair.publicKey);
        var private_key = this.util.base64RawURLEncode(keypair.privateKey);
        return {
          "public": public_key,
          "private": private_key
        };
      ...
      key: "privateKeyToCurve25519",
      value: function privateKeyToCurve25519(privateKey) {
        var seed = privateKey.subarray(0, 32);
        var md = forge__default["default"].md.sha512.create();
        md.update(seed.toString('binary'));
        var digestx = md.digest();
        var digest = Buffer.from(digestx.getBytes(), 'binary');
        digest[0] &= 248;
        digest[31] &= 127;
        digest[31] |= 64;
      ...
      key: "sharedEd25519Key",
      value: function sharedEd25519Key(pinToken, privateKey) {
        pinToken = Buffer.from(pinToken, 'base64');
        privateKey = Buffer.from(privateKey, 'base64');
        privateKey = this.privateKeyToCurve25519(privateKey);
        return curve25519Js.sharedKey(privateKey, pinToken);
      }
    }, {
      key: "signEd25519PIN",
      value: function signEd25519PIN(pin, pinToken, sessionId, privateKey, iterator) {
        var blockSize = 16;
        var Uint64;
        ...
        var sharedKey = this.sharedEd25519Key(pinToken, privateKey);
        ...
        var iv = forge__default["default"].random.getBytesSync(16);
        var key = this.hexToBytes(forge__default["default"].util.binary.hex.encode(sharedKey));
        var cipher = forge__default["default"].cipher.createCipher('AES-CBC', key);
        cipher.start({
          iv: iv
        });
      ...
      value: function createUser(fullName, uid, sid, mainPrivateKey, callback) {
        ...
        var sessionSecret = keyPair["public"];
        var sessionKey = keyPair["private"];
        var data = {
          session_secret: sessionSecret,
          full_name: fullName
        };
        var client = new Client();
        return client.request(uid, sid, mainPrivateKey, 'POST', '/users', data).then(function (res) {
          var user = res.data;
          var userData = {
            user: user,
      ...
      key: "verifyPin",
      value: function verifyPin(pin, pinToken, uid, sid, sessionKey, callback) {
        var encryptedPin = this.signEd25519PIN(pin, pinToken, sid, sessionKey);
        var data = {
          pin: encryptedPin
        };
        return this.client.request(uid, sid, sessionKey, 'POST', '/pin/verify', data).then(function (res) {
          if (callback) {
            callback(res.data);
          } else {
            return res.data;
...
})(forge, LittleEndian, curve25519Js, uuid, axios, jsSHA);
